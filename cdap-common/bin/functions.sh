...
cdap_home() {
  ...
  local readonly __zero=${0}
  local __ls __link __prg
...
# checks if a given PID is running
cdap_status_pidfile() {
...
# Kill a given process by PID file
cdap_stop_pidfile() {
...
# checks for a given version of Node.js in the PATH
cdap_check_node_version() {
  ...
  local readonly __maj=$(echo ${__node} | awk -F'[\\\.v]*' '{ print $2 }')
  local readonly __min=$(echo ${__node} | awk -F'[\\\.v]*' '{ print $3 }')
  local readonly __pat=$(echo ${__node} | awk -F'[\\\.v]*' '{ print $4 }')
...
# This is used by CDAP UI's config-tool, so verify that changes here do not break that
cdap_set_java () {
  local readonly __java
...
cdap_set_classpath() {
  COMP_HOME=${1}
  CCONF=${2}
  # TODO: check for HBASE_CLASSPATH env
...
cdap_sdk_start() {
  ...
  # Start SDK processes
  echo -n "Starting CDAP Standalone (SDK) ..."
  if ${__foreground}; then
    echo
    ...
    while kill -0 ${__pid} 2>&1 >/dev/null; do
      if grep '..* started successfully' "${LOG_DIR}"/cdap.log 2>&1 >/dev/null; then
        if ${__debug}; then
          echo; echo "Remote debugger agent started on port ${__port}"
        else
          echo
        fi
        grep -A 1 '..* started successfully' "${LOG_DIR}"/cdap.log
        break
      elif grep 'Failed to start server' "${LOG_DIR}"/cdap.log 2>&1 >/dev/null; then
    ...
    echo
    if ! kill -0 ${__pid} 2>&1 >/dev/null; then
...
cdap_service() {
  ...
  cdap_${__service} ${__action} ${__args}
# FOO
}
...
_start_bin() {
  ...
  logecho "$(date) Starting ${CDAP_SERVICE} service on ${HOSTNAME}"
  ulimit -a >>${__logfile} 2>&1
  nohup nice -n ${NICENESS} ${MAIN_CMD} ${MAIN_CMD_ARGS} </dev/null >>${__logfile} 2>&1 &
  __pid=${!}
  __ret=${?}
  echo $${__pid} >${__pidfile}
  if ! kill -0 ${__pid} 2>&1 >/dev/null; then
...
_start_java() {
  ...
  # Setup classpaths.
  cdap_set_classpath "${CDAP_HOME}"/${CDAP_SERVICE} "${CDAP_CONF}"
  # Setup Java
  ...
  if [ "${CDAP_SERVICE}" == "master" ]; then
    ...
    logecho "$(date) Running startup checks -- this may take a few minutes"
  ...
  logecho "$(date) Starting Java ${CDAP_SERVICE} service on ${HOSTNAME}"
  "${JAVA}" -version 2>>${__logfile}
  ulimit -a >>${__logfile}
  logecho "CLASSPATH=${CLASSPATH}"
  __defines+=" ${OPTS}"
  logecho "Running: ${JAVA} ${__defines} -cp ${CLASSPATH} ${MAIN_CLASS} ${MAIN_CLASS_ARGS} ${@}"
  # Start our JVM
...
cdap_auth_server() {
  ...
  local EXTRA_CLASSPATH="/etc/hbase/conf/"
  echo "CDAP Auth Server: ${@}"
  _start_java || die "Failed to start CDAP ${CDAP_SERVICE} service"
}
...
cdap_ui() {
  ...
  echo "CDAP UI: ${@}"
  _start_bin || die "Failed to start CDAP ${CDAP_SERVICE} service"
}
...
# Set and export CDAP_HOME
export CDAP_HOME=${CDAP_HOME:-/opt/cdap}
